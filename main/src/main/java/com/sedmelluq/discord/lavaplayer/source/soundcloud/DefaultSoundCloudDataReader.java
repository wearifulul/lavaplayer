...
public class DefaultSoundCloudDataReader implements SoundCloudDataReader {
  ...
  public AudioTrackInfo readTrackInfo(JsonBrowser trackData, String identifier) {
    String artwork = trackData.get("artwork_url") != null
    ? trackData.get("artwork_url").text().replace("large", "t500x500")
    : trackData.get("user").get("avatar_url") != null
    ? trackData.get("user").get("avatar_url").text().replace("large", "t500x500")
    : "https://i1.sndcdn.com/avatars-HvS8x3gDzSME3LpE-ZQfuew-t500x500.jpg";
